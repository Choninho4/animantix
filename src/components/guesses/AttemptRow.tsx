...
export function AttemptRow({ guess, isBest, justAdded }: AttemptRowProps) {
  ...
  return (
    <motion.li
      ...
      className={`flex items-center gap-3 rounded-card border bg-surface px-3 py-2.5 ${
        justAdded ? 'motion-safe:animate-amx-flash' : ''
      }`}
    >
      <span className="min-w-0 flex-1">
        <span className="block truncate text-[15px] font-bold text-text">{guess.nom}</span>
